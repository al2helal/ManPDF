...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={},
            pdftitle={asn1\_der\_decoding(3)},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\toprule\addlinespace
asn1\_der\_decoding(3) & libtasn1 & asn1\_der\_decoding(3)
\\\addlinespace
\bottomrule
\end{longtable}
...
asn1\_der\_decoding - API function
...
\textbf{int asn1\_der\_decoding(asn1\_node *}\emph{element}\textbf{,
const void *}\emph{ider}\textbf{, int}\emph{ider\_len}\textbf{, char
...
Fill the structure * \emph{ELEMENT} with values of a DER encoding
string. The structure must just be created with function
\textbf{asn1\_create\_element()}. If an error occurs during the decoding
procedure, the * \emph{ELEMENT} is deleted and set equal to
\textbf{NULL}.

\hyperdef{}{RETURNS}{\section{\hyperref[RETURNS]{RETURNS}}\label{RETURNS}}

\textbf{ASN1\_SUCCESS} if DER encoding OK,
\textbf{ASN1\_ELEMENT\_NOT\_FOUND} if \emph{ELEMENT} is \textbf{NULL},
and \textbf{ASN1\_TAG\_ERROR} or \textbf{ASN1\_DER\_ERROR} if the der
encoding doesn't match the structure name (* \emph{ELEMENT} deleted).
