...
\toprule\addlinespace
asn1\_array2tree(3) & libtasn1 & asn1\_array2tree(3)
\\\addlinespace
\bottomrule
\end{longtable}
...
\textbf{int asn1\_array2tree(const asn1\_static\_node
*}\emph{array}\textbf{, asn1\_node *}\emph{definitions}\textbf{, char
...
\item[const asn1\_static\_node * array]
...
\emph{array} is a vector created by \textbf{asn1\_parser2array()}.

\hyperdef{}{RETURNS}{\section{\hyperref[RETURNS]{RETURNS}}\label{RETURNS}}

\textbf{ASN1\_SUCCESS} if structure was created correctly,
\textbf{ASN1\_ELEMENT\_NOT\_EMPTY} if * \emph{definitions} not NULL,
\textbf{ASN1\_IDENTIFIER\_NOT\_FOUND} if in the file there is an
identifier that is not defined (see \emph{errorDescription} for more
information), \textbf{ASN1\_ARRAY\_ERROR} if the array pointed by
\emph{array} is wrong.
