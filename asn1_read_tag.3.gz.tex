...
\toprule\addlinespace
asn1\_read\_tag(3) & libtasn1 & asn1\_read\_tag(3)
\\\addlinespace
\bottomrule
\end{longtable}
...
\textbf{int asn1\_read\_tag(asn1\_node}\emph{root}\textbf{, const char
*}\emph{name}\textbf{, int *}\emph{tagValue}\textbf{, int
*}\emph{classValue}\textbf{);}
...
Returns the TAG and the CLASS of one element inside a structure. CLASS
can have one of these constants: \textbf{ASN1\_CLASS\_APPLICATION},
\textbf{ASN1\_CLASS\_UNIVERSAL}, \textbf{ASN1\_CLASS\_PRIVATE} or
\textbf{ASN1\_CLASS\_CONTEXT\_SPECIFIC}.
...
if \\ \emph{name} is not a valid element.
